...
public class TextViewAd extends TextView {
    ...
    private List<IndexInfoEntity.DataBean.GundongConfigBean> mTexts; //显示文字的数据源
...
//        mPaintFront.setTextSize(30);
        ...
        mPaintBack.setTextSize(screenWidth/30);
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mTexts != null) {
            //获取当前的数据
            final IndexInfoEntity.DataBean.GundongConfigBean gundongConfigBean = mTexts.get(mIndex);
            String back = gundongConfigBean.getName();
...
//            mPaintFront.getTextBounds(font, 0, font.length(), indexBound);
            ...
            canvas.drawText(back, 0, back.length(), 20, mY, mPaintBack);
